...
$thisurl = new moodle_url('/blocks/tbs/web/admin.php', array('day' => $day, 'month' => $month, 'year' => $year));
if ($area) {
...
print_header_tbs($day, $month, $year, isset($area) ? $area : "");

// If area is set but area name is not known, get the name.
if ($area) {
    if (empty($area_name)) {
        $dbarea = $DB->get_record('block_tbs_area', array('id' => $area), 'area_name', MUST_EXIST);
...
echo '<table border=1>';
echo '<tr>';
echo '<th><center><b>'.get_string('areas', 'block_tbs').'</b></center></th>';
echo '<th><center><b>'.get_string('rooms', 'block_tbs').' ';
if (isset($area_name)) {
    echo get_string('in', 'block_tbs')." ".s($area_name);
}
...
if (empty($areas)) {
    echo get_string('noareas', 'block_tbs');
} else {
    echo '<ul>';
    foreach ($areas as $dbarea) {
        $area_name_q = urlencode($dbarea->area_name);
        $adminurl = new moodle_url('/blocks/tbs/web/admin.php', array(
            'area' => $dbarea->id, 'area_name' => $area_name_q, 'sesskey' => sesskey()
        ));
        $editroomurl = new moodle_url('/blocks/tbs/web/edit_area_room.php', array('area' => $dbarea->id, 'sesskey' => sesskey()));
        $delareaurl = new moodle_url('/blocks/tbs/web/del.php', array(
...
if ($area) {
    ...
    if (empty($rooms)) {
        ...
        echo get_string('norooms', 'block_tbs');
    } else {
        echo '<ul>';
        foreach ($rooms as $dbroom) {
            $editroomurl = new moodle_url('/blocks/tbs/web/edit_area_room.php', array(
                'room' => $dbroom->id, 'sesskey' => sesskey()
            ));
            $delroomurl = new moodle_url('/blocks/tbs/web/del.php', array(
    ...
    echo get_string('noarea', 'block_tbs');
}

$addareaurl = new moodle_url('/blocks/tbs/web/add.php', array('type' => 'area', 'sesskey' => sesskey()));
$addroomurl = new moodle_url($addareaurl, array('type' => 'room', 'area' => $area));

echo '</tr><tr><td class="border"><h3 ALIGN=CENTER>'.get_string('addarea', 'block_tbs').'</h3>';
...
echo '<input type=submit value="'.get_string('addarea', 'block_tbs').'"></form></td><td class="border">';
if (0 != $area) {
    echo '<h3 align=center>'.get_string('addroom', 'block_tbs').'</h3>';
    ...
    echo '<tr><td>'.get_string('capacity', 'block_tbs').': </td><td><input type="text" name="capacity"></td></tr>';
    echo '</table><input type=submit value="'.get_string('addroom', 'block_tbs').'"></form>';
} else {
    echo "&nbsp;";
}
echo '</td></tr></table>';
//echo '<br />'.get_string('browserlang','block_tbs').' '.$HTTP_ACCEPT_LANGUAGE.' '.get_string('postbrowserlang','block_tbs');
include 'trailer.php';
