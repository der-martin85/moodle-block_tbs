...
#################################
$tbs_admin = $cfg_tbs->admin;  //moodle username of the tbs administrator
$tbs_admin_email = $cfg_tbs->admin_email; // "admin@MyMoodleSite.com";
...
# $tbs_company = "<a href=http://www.your_organisation.com/>
# <img src=your_logo.gif border=0></a>";
$tbs_company = $SITE->fullname;
$tbs_company_url = $CFG->wwwroot;

# This is to fix URL problems when using a proxy in the environment.
# If links inside TBS appear broken, then specify here the URL of
# your TBS root directory, as seen by the users. For example:
...
$enable_periods = $cfg_tbs->enable_periods;
...
# morningstarts must be < eveningends. See also eveningends_minutes.
    $morningstarts = $cfg_tbs->morningstarts;
    $eveningends   =$cfg_tbs->eveningends;
...
# and eveningends_minutes = 30
    $morningstarts_minutes = $cfg_tbs->morningstarts_min;
...
# eveningends_minutes=45; and resolution=900.
    $eveningends_minutes = $cfg_tbs->eveningends_min;
}
...
$max_rep_entrys = $cfg_tbs->max_rep_entrys + 1;

# Max advance days for booking
$max_advance_days = isset($cfg_tbs->max_advance_days) ? $cfg_tbs->max_advance_days : -1;

# Default report span in days:
$default_report_days = $cfg_tbs->default_report_days;

# Results per page for searching:
$search["count"] = $cfg_tbs->search_count;

# Page refresh time (in seconds). Set to 0 to disable
//$refresh_rate = $cfg_tbs->refresh_rate;

# should areas be shown as a list or a drop-down select box?
$area_list_format = $cfg_tbs->area_list_format;
...
# of 12.
$monthly_view_entries_details = $cfg_tbs->monthly_view_entries_details;

# To view weeks in the bottom (trailer.php) as week numbers (42) instead of
# 'first day of the week' (13 Oct), set this to TRUE
$view_week_number = $cfg_tbs->view_week_number;

# To display times on right side in day and week view, set to TRUE;
$times_right_side = $cfg_tbs->times_right_side;

# Control the active cursor in day/week/month views.
$javascript_cursor = $cfg_tbs->javascript_cursor; # Change to false if clients have old browsers
                           # incompatible with JavaScript.
$show_plus_link = $cfg_tbs->show_plus_link; # Change to true to always show the (+) link as in
                        # TBS 1.1.
$highlight_method = $cfg_tbs->highlight_method; # One of "bgcolor", "class", "hybrid".

# Define default starting view (month, week or day)
# Default is day
$default_view = $cfg_tbs->default_view;
...
# Default is 0
$default_room = $cfg_tbs->default_room;
...
} else { //if one is set in Moodle then use that path for TBS block cookies
...
# FALSE
define ("MAIL_ADMIN_ON_BOOKINGS", $cfg_tbs->mail_admin_on_bookings);
...
define ("MAIL_AREA_ADMIN_ON_BOOKINGS", $cfg_tbs->mail_area_admin_on_bookings);
...
define ("MAIL_ROOM_ADMIN_ON_BOOKINGS", $cfg_tbs->mail_room_admin_on_bookings);

# Set to TRUE if you want ADMIN to be notified when entries are deleted. Email
# will be sent to tbs admin, area admin and room admin as per above settings,
# as well as to booker if MAIL_BOOKER is TRUE (see below).
define ("MAIL_ADMIN_ON_DELETE", $cfg_tbs->mail_admin_on_delete);
...
define ("MAIL_ADMIN_ALL", $cfg_tbs->mail_admin_all);
...
# FALSE.
define ("MAIL_DETAILS", $cfg_tbs->mail_details);
...
# how to set tbs to send emails to users/bookers, see INSTALL.
define ("MAIL_BOOKER", $cfg_tbs->mail_booker);
...
# Set the email address of the From field. Default is $tbs_admin_email
define ("MAIL_FROM", $cfg_tbs->mail_from);

# Set the recipient email. Default is $tbs_admin_email. You can define
# more than one recipient like this "john@doe.com,scott@tiger.com"
define ("MAIL_RECIPIENTS", $cfg_tbs->mail_recipients);

# Set email address of the Carbon Copy field. Default is ''. You can define
# more than one recipient (see MAIL_RECIPIENTS)
define ("MAIL_CC", $cfg_tbs->mail_cc);
...
$unicode_encoding = 1; //this should not be an option in Moodle TBS block

# Set this to a different language specifier to default to different
# language tokens. This must equate to a lang.* file in TBS.
# e.g. use "fr" to use the translations in "lang.fr" as the default
# translations
// TODO: Evaluate how TBS uses locale, convert make TBS block more consistent with Moodle approach to locales
if (isset($USER->lang)) {
...
# Set this to 1 to disable the automatic language changing TBS performs
...
# as specified above
$disable_automatic_language_changing = 1; //this should not be an option in Moodle TBS block

# Set this to a valid locale (for the OS you run the TBS server on)
# if you want to override the automatic locale determination TBS
# performs
$override_locale = ""; //this should not be an option in Moodle TBS block, let's keep it simple and just use Moodle's $USER->lang
...
# Each type has a color (see TD.x classes in the style sheet tbs.css).
...
$typel["A"] = $cfg_tbs->entry_type_a;
$typel["B"] = $cfg_tbs->entry_type_b;
$typel["C"] = $cfg_tbs->entry_type_c;
$typel["D"] = $cfg_tbs->entry_type_d;
$typel["E"] = $cfg_tbs->entry_type_e;
$typel["F"] = $cfg_tbs->entry_type_f;
$typel["G"] = $cfg_tbs->entry_type_g;
$typel["H"] = $cfg_tbs->entry_type_h;
$typel["I"] = $cfg_tbs->entry_type_i;
$typel["J"] = $cfg_tbs->entry_type_j;

//These are here so that unchanged imported bookings can be replaced on new imports
if (!empty($cfg_tbs->cronfile)) {
    $typel["K"] = get_string('importedbooking','block_tbs');
    $typel["L"] = get_string('importedbookingmoved','block_tbs');
}
$typel["U"] = get_string('unconfirmedbooking', 'block_tbs');
