 ...
 */
function tbsDelEntry($user, $id, $series, $all, $roomadminoverride = false) {
    global $DB;
    ...
    $removed = 0;
    $entries = $DB->get_records('block_tbs_entry', $params);
    foreach ($entries as $entry) {
        ...
        $DB->delete_records('block_tbs_entry', array('id' => $entry->id));

        $removed++;
    }

    if ($repeat_id > 0 && $DB->count_records('block_tbs_entry', array('repeat_id' => $repeat_id)) == 0) {
        $DB->delete_records('block_tbs_repeat', array('id' => $repeat_id));
    }

    return $removed > 0;
}

/** tbsCreateSingleEntry()
 *
 ...
 */
function tbsCreateSingleEntry($starttime, $endtime, $entry_type, $repeat_id, $room_id,
    ...
    if ($endtime > $starttime) {
        if ($oldid) {
            $add->id = $oldid;
            $DB->update_record('block_tbs_entry', $add);
            return $oldid;
        } else {
            return $DB->insert_record('block_tbs_entry', $add);
        }
    }
 ...
 *   non-zero - The entry's ID
 */
function tbsCreateRepeatEntry($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt,
    ...
    if ($oldrepeatid) {
        $add->id = $oldrepeatid;
        $DB->update_record('block_tbs_repeat', $add);
    } else {
        $add->id = $DB->insert_record('block_tbs_repeat', $add);
    }
 ...
 */
function tbsGetRepeatEntryList($time, $enddate, $rep_type, $rep_opt, $max_ittr, $rep_num_weeks) {
    $sec = date("s", $time);
    $min = date("i", $time);
    $hour = date("G", $time);
    $day = date("d", $time);
 ...
 *   non-zero - The entry's ID
 */
function tbsCreateRepeatingEntrys($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt,
    ...
    $repeatid = 0;
    if ($oldid) {
        $repeatid = $DB->get_field('block_tbs_entry', 'repeat_id', array('id' => $oldid));
    }

    if (empty($reps)) {
        if ($repeatid) {
            // This was a repeat entry, but the entry no longer has any repeats, so delete the repeats (but not this entry)
            $DB->delete_records_select('block_tbs_entry', 'repeat_id = :repeatid AND id <> :oldid', compact('repeatid', 'oldid'));
            $DB->delete_records('block_tbs_repeat', array('id' => $repeatid));
        }

        $ret->id = tbsCreateSingleEntry($starttime, $endtime, 0, 0, $room_id, $owner, $name, $type, $description, $oldid, $roomchange);
        $ret->repeating = 0;
        $ret->requested = 1;
        $ret->created = 1;
    ...
    $ret->id = tbsCreateRepeatEntry($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt, $room_id, $owner, $name, $type, $description, $rep_num_weeks, $repeatid);
    ...
    if ($ret->id) {
        ...
        if ($repeatid) {
            ...
            $oldids = $DB->get_fieldset_sql('SELECT id FROM {block_tbs_entry} WHERE repeat_id = ? ORDER BY start_time', array($repeatid));
        }
        ...
        for ($i = 0; $i < count($reps); $i++) {
            ...
            tbsCreateSingleEntry($reps[$i], $reps[$i] + $diff, 1, $ret->id,
        ...
        for ($i = count($reps); $i < count($oldids); $i++) {
            $DB->delete_records('block_tbs_entry', array('id' => $oldids[$i]));
        }
    }
    return $ret;
}
 ...
 */
function tbsGetEntryInfo($id) {
    global $DB;
    return $DB->get_record('block_tbs_entry', array('id' => $id));
}

function tbsGetRoomArea($id) {
    global $DB;
